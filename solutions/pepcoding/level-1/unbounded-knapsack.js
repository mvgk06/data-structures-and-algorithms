...
1. Top down
- The memo[i][j] represents the maximum value that can be obtained if the capacity is j and elements till ith index can be used.
...
- To compute the solutions for the sub problems in the current row, we need the solutions only from the previous row.
...
const knapsack = (profits, weights, capacity, index, memo) => {
	...
	if (weights[index] <= capacity) {
		const pick =
			profits[index] + knapsack(profits, weights, capacity - weights[index], index, memo);
