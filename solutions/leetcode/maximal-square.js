...
1. Top down
- Each state in memo[i][j] represents the maximum side of valid square that ends at (i,j).
...
- The result is maximum square of all elements in memo.
...
const maximalSquare3 = function (matrix) {
    const m = matrix.length, n = matrix[0].length;
    ...
    for (let i = 0; i < m; i++) {
        ...
        for (let j = 0; j < n; j++) {
            ...
            result = Math.max(result, curr[j] * curr[j]);
        }
        prev = [...curr];
    }
    return result;
